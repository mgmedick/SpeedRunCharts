    ...
    <div v-if="!loading" id="divGridContainer" class="container-lg ml-lg-2 p-0">
        ...
        <div v-for="(categoryType, categoryTypeIndex) in categoryTypes" :key="categoryType.id">
            <div v-if="categoryTypeID == categoryType.id">
                <!--<personalbest-grid :userid="userid" :categorytypeid="categoryTypeID.toString()"></personalbest-grid>-->
            </div>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods: {
            loadData() {
                ...
                var prms = axios.get('../Game/GetPersonalBestGridTabs', { params: { userID: this.userid } })
                                .then(res => {
