...
<script>
    ...
    export default {
        name: "UserSpeedRunCountDonutChart",
        ...
        computed: {   
            ...
            caption: function () {
                return (this.categorytypeid == 0 ? 'Game' : 'Level') + ' Runs Distribution';
            },                            
                ...
                if (this.tabledata?.length > 0) {
                    ...
                    if (this.categorytypeid == 0) {
                        this.games.filter(i => i.categories.filter(i=>i.categoryTypeID == that.categorytypeid).length > 0).forEach(game => {
                            ...
                            var gameData = _alldata.filter(i => i.gameID == game.id);
                            var gameCount = gameData.length;

                            if(gameCount > 0) {
                                chartObj[gameName] = { count: gameCount };

                                game.categories.filter(i => i.categoryTypeID == that.categorytypeid).forEach(category => {
                                    var categoryName = category.name.replace(/( \(empty\)$)/g, '');
                                    var categoryData = gameData.filter(i => i.categoryID == category.id);
                                    var categoryCount = categoryData.length;

                                    if (categoryCount > 0) {
                                        chartObj[gameName][categoryName] = { count: categoryCount };

                                        if (game.subCategoryVariablesTabs && game.subCategoryVariablesTabs.filter(i => i.categoryID == category.id).length > 0) {
                                            that.setVariableChartData(game.subCategoryVariablesTabs.filter(i => i.categoryID == category.id), categoryData, chartObj[gameName][categoryName]);                                 
                                        }  
                                    }
                                })
                            }
                            ...
                            if(gameCount > 0) {
                                ...
                                game.categories.filter(i => i.categoryTypeID == that.categorytypeid).forEach(category => {
                                    ...
                                    if (categoryCount > 0) {
                                        ...
                                        game.levels.filter(i => i.categoryID == category.id).forEach(level => {
                                            var levelName = level.name.replace(/( \(empty\)$)/g, '');
                                            var levelData = categoryData.filter(i => i.categoryID == category.id && i.levelID == level.id);
                                            var levelCount = levelData.length;

                                            if (levelCount > 0) {
                                                chartObj[gameName][categoryName][levelName] = { count: levelCount };

                                                if (game.subCategoryVariablesTabs && game.subCategoryVariablesTabs.filter(i => i.categoryID == category.id && i.levelID == level.id).length > 0) {
                                                    that.setVariableChartData(game.subCategoryVariablesTabs.filter(i => i.categoryID == category.id && i.levelID == level.id), levelData, chartObj[gameName][categoryName][levelName]);                                 
                                                }
                                            }
                                        });  
                                    }
