            ...
            <div v-for="(variableValue, variableValueIndex) in variable.variableValues" :key="variableValue.id">
                <div v-if="subcategoryvariablevalueids[variable.name + variableindex] == variableValue.name">
                    <div v-if="variableValue.subVariables?.filter(v1 => (v1.scopeTypeID == '0' || v1.scopeTypeID == '2')).length > 0">            
                        {{ 'tab loop' }}
                        <game-worldrecord-variable-grid-tab :items="variableValue.subVariables?.filter(v1 => (v1.scopeTypeID == '0' || v1.scopeTypeID == '2'))" :gameid="gameid" :categorytypeid="categorytypeid" :categoryids="categoryids" :levelids="levelids" :singlelevelids="singlelevelids"  :subcategoryvariables="subcategoryvariables" :alllevelvariables="alllevelvariables" :subcategoryvariablevalueids="subcategoryvariablevalueids" :prevdata="(prevdata + ',' + variableValue.id).replace(/(^,)|(,$)/g, '')" :prevdatanames="(prevdatanames + ',' + variableValue.name).replace(/(^,)|(,$)/g, '')" :variableindex="variableindex + 1" :showmilliseconds="showmilliseconds" :variables="variables" @ontabclick="$emit('ontabclick', $event)"></game-worldrecord-variable-grid-tab>
                    </div>
                    <!-- <div v-else-if="variableValue.subVariables && variableValue.subVariables.length > 0">
                        {{ 'gameid: ' + gameid + ' categoryids: ' + categoryids + ' levelids: ' + (levelids ?? '') + ' variablevalues: ' + (prevdata + ',' + variableValue.id).replace(/(^,)|(,$)/g, '') }}
                        <game-worldrecord-variable-grid :items="variableValue.subVariables" :gameid="gameid" :categorytypeid="categorytypeid" :categoryids="categoryids" :levelids="levelids" :subcategoryvariablevalueids="subcategoryvariablevalueids" :prevdata="(prevdata + ',' + variableValue.id).replace(/(^,)|(,$)/g, '')" :variableindex="variableindex + 1" :showmilliseconds="showmilliseconds" :variables="variables"></game-worldrecord-variable-grid>                        
                    </div> -->
                    <div v-else>
                        {{ 'non variable again' }}
                        ...
                        <div v-for="(levelID, levelIndex) in singlelevelids" :key="levelID">
                            ...
                            <!-- <div v-if="subcategoryvariables?.filter(v1 => v1.categoryID == categoryids && v1.levelID == levelID && v1.variableValues.filter(va => va.id == variableValue.id && va.hasData).length > 0).length > 0"> -->
                            <div>
                                <!-- {{ 'gameid: ' + gameid + ' categoryids: ' + categoryids + ' levelids: ' + (levelID ?? '') + ' variablevalues: ' + getLevelVariableValueIDs(categoryids, levelID, variableValue.name) }} -->
